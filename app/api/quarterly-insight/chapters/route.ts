...
export async function GET(req: NextRequest) {
  try {
    ...
    const aiSynthesisLocked = isQuarterlyInsightFeatureLocked('ai_synthesis', profileUser)
    ...
    const chapters = rows
      ...
      .map((r) => {
        const quarterStart = r.prompt_date!.slice(0, 10)
        const d = parseISO(quarterStart)
        const periodKey = quarterKeyFromDate(d)
        const q = Math.floor(d.getMonth() / 3) + 1
        const periodLabel = `Q${q} ${d.getFullYear()}`
    ...
    console.error('[quarterly-insight/chapters] error', err)
    return NextResponse.json({ error: 'Failed to load quarterly chapters' }, { status: 500 })
  }
}
