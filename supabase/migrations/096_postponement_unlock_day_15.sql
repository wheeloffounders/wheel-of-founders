-- Align Postponement Patterns trigger + data with app schedule: day 15 active (was 7 in 092).
...
-- Remove premature unlocks (account age < 15 days)
...
WHERE unlock_type = 'feature'
  ...
  AND user_id IN (
    SELECT up.id
    FROM public.user_profiles up
    WHERE FLOOR(EXTRACT(EPOCH FROM (now() - up.created_at)) / 86400)::int < 15
  );
...
-- Backfill for eligible users missing the row
INSERT INTO public.user_unlocks (user_id, unlock_type, unlock_name, unlocked_at)
SELECT
  up.id,
  'feature'::text,
...
WHERE FLOOR(EXTRACT(EPOCH FROM (now() - created_at)) / 86400)::int >= 15
