    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 dark:bg-gray-800 p-6 rounded-lg shadow">
        ...
        <div className="flex gap-6 text-sm">
          <div>
            <span className="text-gray-500 dark:text-gray-500 dark:text-gray-400">Sessions</span>
            <div className="text-xl font-bold text-[#152b50] dark:text-[#E2E8F0]">{totalSessions}</div>
          </div>
          <div>
            <span className="text-gray-500 dark:text-gray-500 dark:text-gray-400">Completed flow (Morning→Evening)</span>
            <div className="text-xl font-bold text-[#ef725c]">{completedFlow}</div>
          </div>
          <div>
            <span className="text-gray-500 dark:text-gray-500 dark:text-gray-400">Completion rate</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 dark:bg-gray-800 p-6 rounded-lg shadow">
          ...
          {topPaths.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-500 dark:text-gray-400 text-sm">No paths recorded yet.</p>
          ) : (
            <ul className="space-y-2 text-sm">
              {topPaths.map(([path, count]) => (
                <li key={path} className="flex justify-between">
                  <span className="text-gray-900 dark:text-gray-100 dark:text-gray-200 truncate max-w-[70%]">{path}</span>
        ...
        <div className="bg-white dark:bg-gray-800 dark:bg-gray-800 p-6 rounded-lg shadow">
          ...
          {topDrops.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-500 dark:text-gray-400 text-sm">No drop-off data yet.</p>
          ) : (
            <ul className="space-y-2 text-sm">
              {topDrops.map(([page, count]) => (
                <li key={page} className="flex justify-between">
                  <span className="text-gray-900 dark:text-gray-100 dark:text-gray-200">{page}</span>
