...
import { useMonthlyInsightChapters } from '@/lib/hooks/useMonthlyInsightChapters'
...
type MonthlyInsightChapterArchiveProps = {
  highlightMonthKey?: string | null
}

export function MonthlyInsightChapterArchive({
  highlightMonthKey = null,
}: MonthlyInsightChapterArchiveProps) {
  const router = useRouter()
  const { chapters, loading, error } = useMonthlyInsightChapters()
  const [monthlyNarrativeLocked, setMonthlyNarrativeLocked] = useState(true)
  const [activePeriodKey, setActivePeriodKey] = useState<string | null>(highlightMonthKey)
  ...
  useEffect(() => {
    ...
    const loadLock = async () => {
      ...
      if (cancelled) return
      setMonthlyNarrativeLocked(isMonthlyInsightFeatureLocked('ai_synthesis', profileFromBundle(row)))
    }
    void loadLock()
  ...
  const syncArchiveUrl = useCallback(
    (periodKey: string) => {
      router.replace(insightArchiveHref('month', periodKey), { scroll: false })
    },
    [router],
  )
  ...
  useEffect(() => {
    ...
    const fromUrl =
      highlightMonthKey && chapters.some((c) => c.periodKey === highlightMonthKey)
        ? highlightMonthKey
        : null
    const next = fromUrl ?? chapters[0]!.periodKey
    setActivePeriodKey(next)
    if (!fromUrl) syncArchiveUrl(next)
  }, [chapters, highlightMonthKey, syncArchiveUrl])

  const openMonth = useCallback(
    (periodKey: string) => {
      router.push(`/monthly-insight?month=${periodKey}`)
    },
    [router],
  )
  ...
  const renderChapters = (items: InsightChapterRecord[]) =>
    items.map((c) => (
      <InsightChapterBentoCard
        ...
        record={c}
        contextLabel="MONTH"
        locked={monthlyNarrativeLocked}
        onOpenPeriod={openMonth}
      />
    ))
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm italic text-gray-600 dark:text-gray-400">
        Every saved Mrs. Deer monthly letter, in one place.
        {monthlyNarrativeLocked ? (
          <>
            {' '}
            Free shows your chapter list; open a month to read the reflection teaser, with Pro unlocking the full letter.
          </>
        ) : null}
      </p>
