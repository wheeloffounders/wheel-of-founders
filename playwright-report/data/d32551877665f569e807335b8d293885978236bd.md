# Page snapshot

```yaml
- generic [active] [ref=e1]:
  - navigation [ref=e2]:
    - generic [ref=e4]:
      - link "Wheel of Founders" [ref=e6] [cursor=pointer]:
        - /url: /
        - img "Wheel of Founders" [ref=e7]
      - generic [ref=e8]:
        - link "Dashboard" [ref=e9] [cursor=pointer]:
          - /url: /
          - img [ref=e10]
          - generic [ref=e13]: Dashboard
        - link "Morning" [ref=e14] [cursor=pointer]:
          - /url: /morning
          - img [ref=e15]
          - generic [ref=e21]: Morning
        - link "Emergency" [ref=e22] [cursor=pointer]:
          - /url: /emergency
          - img [ref=e23]
          - generic [ref=e25]: Emergency
        - link "Evening" [ref=e26] [cursor=pointer]:
          - /url: /evening
          - img [ref=e27]
          - generic [ref=e29]: Evening
        - button "Insights" [ref=e31]:
          - img [ref=e32]
          - generic [ref=e33]: Insights
          - img [ref=e34]
        - button "Profile" [ref=e37]:
          - img [ref=e38]
          - generic [ref=e41]: Profile
          - img [ref=e42]
        - button "Toggle dark / light mode" [ref=e44]:
          - img [ref=e45]
  - main [ref=e47]:
    - generic [ref=e49]:
      - heading "Welcome Founder" [level=1] [ref=e50]
      - paragraph [ref=e51]: Sign up or log in to manage your day.
      - generic [ref=e52]:
        - paragraph [ref=e53]: "Continue with:"
        - generic [ref=e54]:
          - button "Continue with Google" [ref=e55]:
            - img [ref=e56]
            - generic [ref=e61]: Continue with Google
          - button "Continue with Apple" [ref=e62]:
            - img [ref=e63]
            - generic [ref=e65]: Continue with Apple
        - generic [ref=e70]: or
      - generic [ref=e71]:
        - generic [ref=e72]:
          - generic [ref=e73]: Email
          - textbox "Email" [ref=e74]:
            - /placeholder: your.email@example.com
        - generic [ref=e75]:
          - generic [ref=e76]: Password
          - textbox "Password" [ref=e77]:
            - /placeholder: ••••••••
        - button "Log In" [disabled] [ref=e78]:
          - img [ref=e79]
          - text: Log In
        - button "Sign Up" [disabled] [ref=e82]:
          - img [ref=e83]
          - text: Sign Up
      - paragraph [ref=e86]:
        - text: Need help?
        - link "Contact support" [ref=e87] [cursor=pointer]:
          - /url: "#"
  - contentinfo [ref=e88]:
    - paragraph [ref=e89]: Install Wheel of Founders as an app on desktop or phone – look for Install app in your browser menu. No app store needed.
  - button "Open Next.js Dev Tools" [ref=e95] [cursor=pointer]:
    - img [ref=e96]
  - alert [ref=e99]: Welcome Founder
```